<?php
/**
 * Bloque personalizado: Hero Section
 */
...
function register_hero_block() {
    register_block_type(__DIR__ . '/hero-block.json');
}
add_action('init', 'register_hero_block');

// Renderizar el bloque
function render_hero_block($attributes) {
    $titulo_linea1 = $attributes['tituloLinea1'] ?? 'DESARROLLO TERRITORIAL REGENERATIVO';
    $titulo_linea2 = $attributes['tituloLinea2'] ?? 'DESDE Y HACIA LATINOAMÉRICA';
    $subtitulo = $attributes['subtitulo'] ?? 'Impulsamos proyectos de desarrollo territorial que devuelven vitalidad a los ecosistemas, fortalecen comunidades y generan prosperidad consciente y regenerativa';
    $texto_boton = $attributes['textoBoton'] ?? 'Conoce Nuestros Servicios';
    $enlace_boton = $attributes['enlaceBoton'] ?? '#servicios';
        ...
        <div style="position:relative;z-index:10;text-align:center;color:white;max-width:900px;margin:0 auto;padding:0 2rem;display:flex;flex-direction:column;justify-content:center;align-items:center;min-height:100vh">
            <h1 class="wp-block-heading" style="font-size:clamp(2.5rem, 6vw, 4rem);font-weight:800;margin:0 0 3rem 0;line-height:1.1;font-family:'Playfair Display',serif;text-shadow:0 6px 12px rgba(0,0,0,0.6), 0 2px 4px rgba(0,0,0,0.4);color:white;letter-spacing:-0.02em">
                <div style="white-space:nowrap;display:inline-block">
                    <div style="display:block;color:white;text-shadow:0 6px 12px rgba(0,0,0,0.6), 0 2px 4px rgba(0,0,0,0.4);white-space:nowrap;margin-bottom:0.5rem"><?php echo esc_html($titulo_linea1); ?></div>
                    <div style="display:block;color:#d3600f;text-shadow:0 6px 12px rgba(0,0,0,0.6), 0 2px 4px rgba(0,0,0,0.4);white-space:nowrap"><?php echo esc_html($titulo_linea2); ?></div>
                </div>
            </h1>
...
register_block_type('sueno-andino/hero-block', array(
