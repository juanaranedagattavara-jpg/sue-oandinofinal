<?php
/**
 * Bloque personalizado: Servicios Section
 */
...
// Registrar el bloque
function register_servicios_block() {
    register_block_type(__DIR__ . '/servicios-block.json');
}
add_action('init', 'register_servicios_block');

// Renderizar el bloque
function render_servicios_block($attributes) {
    $titulo = $attributes['titulo'] ?? 'Soluciones Especializadas';
    $subtitulo = $attributes['subtitulo'] ?? 'Desarrollamos programas integrales que combinan metodologías probadas con innovación constante para cada tipo de organización.';
    $servicios = $attributes['servicios'] ?? [
        [
            'titulo' => 'Para Personas',
            'descripcion' => 'Programas de educación básica y técnica, capacitación en emprendimiento, desarrollo de habilidades digitales y apoyo psicosocial para empoderar a las comunidades andinas.',
            'icono' => 'personas'
        ],
        [
            'titulo' => 'Para Empresas',
            'descripcion' => 'Consultoría en responsabilidad social, programas de desarrollo comunitario, capacitación de personal local y proyectos de impacto social para empresas comprometidas.',
            'icono' => 'empresas'
        ],
        [
            'titulo' => 'Para Gobiernos',
            'descripcion' => 'Políticas públicas de desarrollo, programas de inclusión social, capacitación de funcionarios y evaluación de impacto social para gobiernos locales y nacionales.',
            'icono' => 'gobiernos'
        ]
    ];
                ...
                <div style="position:relative;group">
                    <div style="background:#2a4351;border-radius:2rem;padding:3rem 2.5rem;height:100%;box-shadow:0 0 0 1px rgba(0,0,0,0.04);transition:all 0.6s cubic-bezier(0.4, 0, 0.2, 1);border:1px solid rgba(42,67,81,0.2);position:relative;overflow:hidden" onmouseover="this.style.transform='translateY(-12px)';this.style.boxShadow='0 40px 80px -12px rgba(0,0,0,0.15), 0 0 0 1px rgba(211,96,15,0.1)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 0 0 1px rgba(0,0,0,0.04)'">
                        ...
                        <div style="width:3.5rem;height:3.5rem;background:rgba(211,96,15,0.08);border-radius:1.25rem;display:flex;align-items:center;justify-content:center;margin-bottom:2rem;position:relative;z-index:1">
                            <?php if ($servicio['icono'] === 'personas'): ?>
                            ...
                            <?php elseif ($servicio['icono'] === 'empresas'): ?>
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" style="color:#d3600f">
...
// Registrar el renderizado del bloque
register_block_type('sueno-andino/servicios-block', array(
    'render_callback' => 'render_servicios_block',
));
